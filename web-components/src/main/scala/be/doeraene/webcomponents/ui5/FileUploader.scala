...
object FileUploader extends WebComponent with HasName with HasValue {
  ...
  object events {
    ...
    object FileUploaderFileSizeExceedEventDetail {
      extension (element: FileUploaderFileSizeExceedEventDetail)
        def filesData: List[FileData] = element.filesDataJS.toList
    }

    val onChange: EventProp[EventWithPreciseTarget[Ref] & HasDetail[HasFiles]] = new EventProp("change")
    val onFileSizeExceed: EventProp[EventWithPreciseTarget[Ref] & HasDetail[FileUploaderFileSizeExceedEventDetail]] =
